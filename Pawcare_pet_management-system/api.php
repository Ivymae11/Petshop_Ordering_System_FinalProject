...
function db() {
    ...
    if ((int)$pdo->query("SELECT COUNT(*) FROM categories")->fetchColumn() === 0) {
        ...
        $stmt = $pdo->prepare("
            ...
            (category_name,description,status,created_at)
            VALUES(?,?,?,?)
        ");

        $stmt->execute([
            'Nutrition',
            'Daily meals, treats, and supplements',
            'Active',
            date('Y-m-d H:i:s')
        ]);
        ...
        $stmt->execute([
            'Grooming Care',
            'Shampoo, brushes, coat and hygiene products',
            'Active',
            date('Y-m-d H:i:s')
        ]);
        ...
        $stmt->execute([
            'Play and Comfort',
            'Toys, beds, collars, carriers, and bowls',
            'Active',
            date('Y-m-d H:i:s')
        ]);
        ...
        $stmt->execute([
            'Wellness',
            'Vitamins, hygiene, and pet health essentials',
            'Active',
            date('Y-m-d H:i:s')
        ]);
    }
    ...
    if ((int)$pdo->query("SELECT COUNT(*) FROM products")->fetchColumn() === 0) {
        ...
        $catNutrition = (int)$pdo->query("
            SELECT id FROM categories
            WHERE category_name='Nutrition'
            LIMIT 1
        ")->fetchColumn();

        $catGroom = (int)$pdo->query("
            SELECT id FROM categories
            WHERE category_name='Grooming Care'
            LIMIT 1
        ")->fetchColumn();

        $catPlay = (int)$pdo->query("
            SELECT id FROM categories
            WHERE category_name='Play and Comfort'
            LIMIT 1
        ")->fetchColumn();

        $catWell = (int)$pdo->query("
            SELECT id FROM categories
        ...
        $stmt->execute([
            'DOG-NUT-001',
            'Grain-Free Dog Meal 5kg',
            $catNutrition,
            $sup,
            'Dog',
            'Balanced dry meal for adult dogs',
            1390,
            36,
            8,
            'Active',
        ...
        $stmt->execute([
            'CAT-NUT-002',
            'Tuna Crunch Cat Bites',
            $catNutrition,
            $sup,
            'Cat',
            'Crunchy tuna-flavored cat treats',
            295,
            52,
            10,
            'Active',
        ...
        $stmt->execute([
            'PET-GRM-003',
            'Oatmeal Coat Shampoo',
            $catGroom,
            $sup,
            'Dog/Cat',
            'Gentle shampoo for sensitive coats',
            365,
            30,
            7,
            'Active',
